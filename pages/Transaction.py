...
def setting_sidebar_dataframe():
    ...
    transaction_data = get_user()                                                   # transaction data 
...
# ================================================   setting the sidebar menu ==============================================================
    ...
    # KPi  for pincode
        ...
        with c1:
            fig = px.bar(year,x="Year",y = "Total_transcation",color="Payment_reason",title=" Total Payment Transaction",hover_data="Total_value",color_continuous_scale="Viridis", color_discrete_sequence=["red", "blue", "green", "purple"])
            fig.update_layout(legend=dict(title=dict(text="Payment Reason"), font=dict(size=15)))
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)
            
        with c2:
            fig =px.scatter(state,x = "Year",y ="Total_transcation",size="Total_value",color="Payment_reason",title="YearlyPayment Transaction",color_discrete_sequence=["red", "blue", "green", "purple"])
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)  
        ...
        with c1:
            fig = px.bar(year,x="Year",y = "Total_value",color="Payment_reason",title=" Total Payment Value in India",hover_data="Total_transcation",color_discrete_sequence=["red", "blue", "green", "purple"])
            fig.update_layout(legend=dict(title=dict(text="Payment Reason"), font=dict(size=15)))
            fig.update_xaxes(showgrid =False)
            st.plotly_chart(fig)
        ...
        with c2:
            fig =px.scatter(state,x = "Year",y ="Total_value",size="Total_value",color="Payment_reason",title="Yearly Payment Value",color_discrete_sequence=["red", "blue", "green", "purple"])
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)     
        ...
        c1,c2 = st.columns(2)
        with c1:
            fig = px.bar(state,x="State",y = "Total_transcation",color="Payment_reason",title=" Total State Payment  Transaction ",color_discrete_sequence=["red", "blue", "green", "purple"])
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)
            
        with c2:
            fig =px.scatter(state,x = "State",y ="Total_transcation",size="Total_value",color="Payment_reason",title="State Payment  Transaction",color_discrete_sequence=["red", "blue", "green", "purple"])
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)  
        ...
        with c1:
            fig = px.bar(state,x="State",y = "Total_value",color="Payment_reason",title=" Total State Payment Value in India",color_discrete_sequence=["red", "blue", "green", "purple"])
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)
              
        with c2:
            fig =px.scatter(state,x = "State",y ="Total_value",size="Total_value",color="Payment_reason",title="State Payment value",color_discrete_sequence=["red", "blue", "green", "purple"])
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)
        ...
        with c1:
            #st.write(district)
            fig = px.bar(district,x="District",y = "Total_transactions",color="Total_transactions",title=" Total District Transaction in India",color_continuous_scale="portland")
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)
            
        with c2:
            fig =px.scatter(district,x = "District",y ="Total_transactions",size="Total_transactions",color="Total_transactions",title=" Individual District Transaction ",color_continuous_scale="portland")
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)  
        ...
        with c1:
            fig = px.bar(district,x="District",y = "Total_value",color="Total_value",title=" Total District Value in India",color_continuous_scale="sunsetdark")
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)
        ...
        with c2:
            fig =px.scatter(district,x = "District",y ="Total_value",size="Total_value",color="Total_value",title=" Individual District value",color_continuous_scale="sunsetdark")
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)
        
        
        # Plot for locality using the pincode
        
        with c1:
            #st.write(pincode)
            pincode['Pincode'] = pincode['Pincode'].astype(str)
            
            fig = px.histogram(pincode, x='Pincode',y ="Total_transactions" ,color="State",title="Total Locatity Transaction on each state")
            st.plotly_chart(fig, use_container_width=True) 
            
            fig = px.histogram(pincode, x='Pincode',y ="Total_value" ,color="State",title="Total Locatity Values on Each state")
            st.plotly_chart(fig, use_container_width=True) 
            
            fig = px.histogram(pincode, x='Pincode',y ="Total_transactions" ,color="Pincode",title=" Grouped Total Locatity Transaction ")
            st.plotly_chart(fig, use_container_width=True) 
            
            fig = px.histogram(pincode, x='Pincode',y ="Total_value" ,color="Pincode",title=" Grouped Total Locatity Values")
            st.plotly_chart(fig, use_container_width=True) 
                
        with c2:
            
            fig =px.scatter(pincode ,x = "State",y ="Total_value",size="Total_transactions",color="Year",title="Transactions in each State",hover_data=["Total_value","Year","Total_transactions"])
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)
            
            
            fig =px.scatter(pincode ,x = "State",y ="Total_value",size="Total_value",color="Year",title="Values in each State",hover_data=["Total_value","Year","Total_transactions"])
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)
            
            
            fig =px.scatter(pincode ,x = "Pincode",y ="Total_value",size="Total_transactions",color="Pincode",title="Transactions in each locality",hover_data=["Total_value","Year","Total_transactions"])
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)
            
            fig =px.scatter(pincode ,x = "Pincode",y ="Total_value",size="Total_value",color="Pincode",title="Values in each locality",hover_data=["Total_value","Year","Total_transactions"])
            fig.update_xaxes(showgrid =False)
            fig.update_yaxes(showgrid =False)
            st.plotly_chart(fig)
